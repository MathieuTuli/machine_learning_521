...
def neuralNetwork():
	...
	# numIterations is 3000
	# batchSize is 500
	# numBatches is 30 (stays constant)
	# numEpochs is 3000/30 = 100
	...
	print(max(trainingAccuracy))
	print(max(validationAccuracy))
	print(max(testAccuracy))

	print(min(trainingLoss))
	print(min(validationLoss))
	print(min(testLoss))
	...
	figure = plt.figure()
	axes = plt.gca()
	plt.plot(epochs, trainingAccuracy, "r-", label = 'Training Acc')
	plt.plot(epochs, validationAccuracy, "g-", label = 'Validation Acc')
	plt.plot(epochs, testAccuracy, "b-", label = 'Test Acc')
	plt.xlabel("Number of epochs")
	plt.ylabel("Accuracy")
	plt.legend(loc='best', shadow = True, fancybox = True)
